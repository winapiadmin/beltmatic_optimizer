...
def leastSteps(target: int, allowedNumbers: List[int], allowedOperators: List[str]) -> List[Step]:
    queue = deque()
    visited = set()
        ...
        for num in allowedNumbers:
            for operator in allowedOperators:
                ...
                if newValue is not None and newValue not in visited:
                    ...
                    queue.append((newValue, stepsList + [(currentValue, num, operator, newValue)]))
